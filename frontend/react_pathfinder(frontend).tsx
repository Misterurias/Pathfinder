...
export default function PathfinderUI() {
  ...
  const sendPositionUpdate = async (lat, lng) => {
    ...
    try {
      await fetch('http://localhost:5000/api/update_position', {
        method: 'POST',
  ...
  const findParking = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/find_parking', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const reserveSpot = async (parkingOption) => {
    try {
      const response = await fetch('http://localhost:5000/api/reserve_spot', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const completePayment = async (paymentMethod) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/complete_payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
                ...
                {alternatives.length > 0 && (
                  <div className="mt-4">
                    ...
                    {alternatives.map((alt, i) => (
                      <div key={i} className="border-2 border-gray-200 rounded-lg p-3 mb-2 hover:border-purple-400 transition">
